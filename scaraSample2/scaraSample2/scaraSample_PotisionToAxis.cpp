/**********************************
	...
	本サンプルのビルド・実行には、CP2110/4 HID USB-to-UART インターフェースライブラリ　が必要です。
	SiLabs社のwebページ(下記)より、ソフトウェア本体とドキュメントをダウンロードし、PCにインストールしてください。
	http://jp.silabs.com/products/interface/Pages/CP2110EK.aspx

	PCにインストールしたら、本ソースを含むVisualStudioのプロジェクトのプロパティを開き、下記の設定を行ってください。
		1．「構成(C):」を「すべての構成」に変更
		2．「構成プロパティ」 -> 「VC++ ディレクトリ」をクリックし、「インクルードディレクトリ」にインクルードファイルへのパスを追加
			デフォルトのインストール設定で「C:\SiLabs\MCU\CP2110_4_SDK\Library\Windows\」になります。
		
		3．同じく、「ライブラリディレクトリ」にライブラリへのパスを追加
			上記で設定したインクルードファイルのパスに「x86」「x64」のフォルダがあるので、プロジェクトの対象プラットフォームに合わせてどちらかを選択します。
			プラットフォームの設定の確認は、プロジェクトのプロパティを開き、「プラットフォーム」の項目を確認します。
			Visual Studioはデフォルトで「Win32」の設定になっており、この場合は「x86」の方を使用します。
			
		4．「OK」をクリック
		5．プログラムの実行フォルダに、「SLABHIDDevice.dll」「SLABHIDtoUART.dll」のファイルをコピー
			「ライブラリディレクトリ」で選択したフォルダの方のファイルを使います。


	通信メッセージを送信すると、必ずローカルエコーが発生します。
	モータ角度等ロボットから情報を取得する場合、情報取得用のメッセージをPCから送信したら、
	ReadLocalEcho関数のようにローカルエコーを全て読み出してから、ロボットからの返信メッセージを受信します。
